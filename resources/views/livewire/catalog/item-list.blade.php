        ...
        @foreach ($imageCategories as $category)
            <div key="{{ $category['id'] }}" class="space-y-8">
                ...
                <div
                    ...
                    x-data="{
                        currentSlide: 0,
                        totalSlides: {{ count($category['images']) }}, // cantidad REAL de imágenes
                        timer: null,
                        isPlaying: true,
                        isTransitioning: true,
                
                        next() {
                            // Avanza hasta el slide CLONADO (index = totalSlides)
                        ...
                        prev() {
                            ...
                            // Si estamos en el primer slide, saltamos al clonado y volvemos al último real
                            if (this.currentSlide === 0 && this.totalSlides > 1) {
                        ...
                        startTimer() {
                            if (this.totalSlides <= 1 || !this.isPlaying) return;
                        ...
                        handleTransitionEnd() {
                            // Cuando llegamos al slide CLONADO (index = totalSlides) reseteamos a 0 sin animación
                ...
                >
                    ...
                    <div class="overflow-hidden rounded-2xl relative">
                        ...
                        >
                            ...
                            @foreach ($category['images'] as $image)
                                <div class="w-full flex-shrink-0">
                                    <div key="{{ $image['id'] }}" class="relative group">
                                        ...
                                        <div class="relative w-full h-[60vh] md:h-[70vh] bg-gray-900 border border-gray-700">
                                            <img
                                                src="{{ $image['image_url'] }}"
                                                alt="{{ $image['title'] }}"
                            ...
                            {{-- 4. Slide CLONADO del primero (para loop infinito) --}}
                            @if (count($category['images']) > 0)
                                @php $firstImage = $category['images'][0]; @endphp
                                <div class="w-full flex-shrink-0">
                                    <div class="relative group">
                                        ...
                                        <div class="relative w-full h-[60vh] md:h-[70vh] bg-gray-900 border border-gray-700">
                                            ...
                                            @foreach ($firstImage['hotspots'] as $hotspot)
                                                <button
                                                    type="button"
                                                    wire:click="selectItem({{ $hotspot['item_id'] }})"
                                                    class="absolute transform -translate-x-1/2 -translate-y-1/2 group/hotspot"
